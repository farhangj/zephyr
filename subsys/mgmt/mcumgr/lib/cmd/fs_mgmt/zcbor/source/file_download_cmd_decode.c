...
#include "file_download_cmd_decode.h"
...
static bool decode_file_download_cmd(
		zcbor_state_t *state, struct file_download_cmd *result)
{
	zcbor_print("%s\r\n", __func__);
...
uint_fast8_t cbor_decode_file_download_cmd(
		const uint8_t *payload, size_t payload_len,
		struct file_download_cmd *result,
		size_t *payload_len_out)
{
	...
	bool ret = decode_file_download_cmd(states, result);
