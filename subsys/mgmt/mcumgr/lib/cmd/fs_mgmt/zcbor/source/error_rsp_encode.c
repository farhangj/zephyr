...
static bool encode_error_rsp(
		zcbor_state_t *state, const struct error_rsp *input)
{
	zcbor_print("%s\r\n", __func__);
	...
	&& (zcbor_int32_encode(state, (&(*input).rc))))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 1))));
		...
		size_t *payload_len_out)
{
	...
	bool ret = encode_error_rsp(states, input);
