...
#include "echo_error_rsp_decode.h"
...
static bool decode_echo_error_rsp(
		zcbor_state_t *state, struct echo_error_rsp *result)
{
	zcbor_print("%s\r\n", __func__);
...
uint_fast8_t cbor_decode_echo_error_rsp(
		const uint8_t *payload, size_t payload_len,
		struct echo_error_rsp *result,
		size_t *payload_len_out)
{
	...
	bool ret = decode_echo_error_rsp(states, result);
