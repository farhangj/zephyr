...
static bool encode_echo_rsp(
		zcbor_state_t *state, const struct echo_rsp *input)
{
	zcbor_print("%s\r\n", __func__);
	...
	&& (zcbor_tstr_encode(state, (&(*input).r))))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 1))));
...
uint_fast8_t cbor_encode_echo_rsp(
		uint8_t *payload, size_t payload_len,
		const struct echo_rsp *input,
		size_t *payload_len_out)
{
	...
	bool ret = encode_echo_rsp(states, input);
