...
static bool decode_echo_rsp(
		zcbor_state_t *state, struct echo_rsp *result)
{
	zcbor_print("%s\r\n", __func__);
	...
	&& (zcbor_tstr_decode(state, (&(*result).r))))) || (zcbor_list_map_end_force_decode(state), false)) && zcbor_map_end_decode(state))));
...
uint_fast8_t cbor_decode_echo_rsp(
		const uint8_t *payload, size_t payload_len,
		struct echo_rsp *result,
		size_t *payload_len_out)
{
	...
	bool ret = decode_echo_rsp(states, result);
